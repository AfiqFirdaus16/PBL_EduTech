...
<div class="flex flex-wrap items-end justify-between gap-3 mb-4">
    <div class="flex flex-wrap items-end gap-3">
        ...
        <div class="flex flex-col gap-1">
            ...
            <div class="relative">
                <select id="filterVar"
                    ...
                    <option value="attendance"          {{ request('sort_by') == 'attendance'          ? 'selected' : '' }}>Kehadiran</option>
                    <option value="sleep_hours"         {{ request('sort_by') == 'sleep_hours'         ? 'selected' : '' }}>Jam Tidur</option>
                    <option value="hours_studied"       {{ request('sort_by') == 'hours_studied'       ? 'selected' : '' }}>Jam Belajar</option>
                    <option value="access_to_resources" {{ request('sort_by') == 'access_to_resources' ? 'selected' : '' }}>Akses Belajar</option>
                    <option value="motivation_level"    {{ request('sort_by') == 'motivation_level'    ? 'selected' : '' }}>Motivasi</option>
                    <option value="tutoring_sessions"   {{ request('sort_by') == 'tutoring_sessions'   ? 'selected' : '' }}>Sesi Les</option>
                    <option value="previous_scores"     {{ request('sort_by') == 'previous_scores'     ? 'selected' : '' }}>Nilai Sebelumnya</option>
                    <option value="kesulitan_belajar"   {{ request('sort_by') == 'kesulitan_belajar'   ? 'selected' : '' }}>Kesulitan Belajar</option>
                </select>
        ...
        <!-- Reset -->
        @if(request()->hasAny(['risk_level', 'sort_by']))
        <div class="flex flex-col gap-1">
            <label class="text-[11px] font-semibold text-transparent uppercase tracking-wide select-none">-</label>
    ...
    <!-- Export -->
    <a href="{{ route('admin.hasil-pengguna.export', request()->only(['risk_level', 'sort_by'])) }}"
...
<!-- Badge filter aktif -->
@if(request('sort_by') || request('risk_level'))
<div class="mb-3 flex items-center gap-2 flex-wrap">
    <span class="text-[12px] text-gray-500">Filter aktif:</span>
    @if(request('sort_by'))
    <span class="inline-flex items-center px-3 py-1 rounded-full bg-indigo-100 text-indigo-700 text-[12px] font-semibold">
        Urut: {{ collect([
...
<div class="bg-white rounded-xl shadow-sm">
    <div class="hasil-table-wrapper" style="overflow-x:auto; position:relative;">
        <table class="hasil-table w-full text-[12px] text-gray-700" style="border-collapse:separate; border-spacing:0; min-width:1300px;">
            <thead>
                <tr class="border-b border-gray-200 text-center text-[12px] font-semibold text-gray-600 bg-white">
                    ...
                    <th class="col-fixed-left px-3 py-3 text-center" style="left:188px; min-width:110px; border-right:2px solid #e5e7eb;">Username</th>

                    <!-- SCROLLABLE MIDDLE — kolom aktif di-highlight -->
                    ...
                    <!-- FIXED RIGHT -->
                    <th class="col-fixed-right px-3 py-3" style="right:360px; min-width:110px; border-left:2px solid #e5e7eb;">Tingkat Risiko</th>
                    <th class="col-fixed-right px-3 py-3 text-left bg-purple-50" style="right:240px; min-width:120px;">Rekomendasi 1</th>
                    <th class="col-fixed-right px-3 py-3 text-left bg-purple-50" style="right:120px; min-width:120px;">Rekomendasi 2</th>
                    <th class="col-fixed-right px-3 py-3 text-left bg-purple-50" style="right:0;    min-width:120px;">Rekomendasi 3</th>
                </tr>
            </thead>
            <tbody id="tableHasil">
                @forelse($hasilAnalisa as $index => $item)
                    ...
                    <tr class="border-b border-gray-100 text-center hover:bg-gray-50 transition">
                        <!-- FIXED LEFT -->
                        <td class="col-fixed-left px-3 py-3 bg-white" style="left:0;">{{ $hasilAnalisa->firstItem() + $index }}</td>
                        <td class="col-fixed-left px-3 py-3 text-left font-medium bg-white" style="left:48px;">{{ $item->sesiKuis->siswa->nama ?? '-' }}</td>
                        <td class="col-fixed-left px-3 py-3 bg-white" style="left:188px; border-right:2px solid #e5e7eb;">{{ $item->sesiKuis->siswa->user->username ?? '-' }}</td>

                        <!-- SCROLLABLE MIDDLE — kolom aktif di-highlight -->
                        <td class="px-3 py-3 {{ request('sort_by') == 'attendance'          ? 'bg-indigo-50 font-semibold' : '' }}">{{ $item->attendance ?? '-' }}</td>
                        <td class="px-3 py-3 {{ request('sort_by') == 'sleep_hours'         ? 'bg-indigo-50 font-semibold' : '' }}">{{ $item->sleep_hours ?? '-' }}</td>
                        <td class="px-3 py-3 {{ request('sort_by') == 'hours_studied'       ? 'bg-indigo-50 font-semibold' : '' }}">{{ $item->hours_studied ?? '-' }}</td>
                        ...
                        <td class="px-3 py-3 {{ request('sort_by') == 'previous_scores'     ? 'bg-indigo-50 font-semibold' : '' }}">{{ $item->previous_scores ?? '-' }}</td>
                        <td class="px-3 py-3 {{ request('sort_by') == 'kesulitan_belajar'   ? 'bg-indigo-50 font-semibold' : '' }}">{{ $item->kesulitan_belajar ?? '-' }}</td>

                        <!-- FIXED RIGHT -->
                        <td class="col-fixed-right px-3 py-3 bg-white" style="right:360px; border-left:2px solid #e5e7eb;">
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-semibold {{ $riskStyle }}">
                                {{ $item->risk_level }}
                            </span>
                        </td>
                        <td class="col-fixed-right px-3 py-3 text-left bg-purple-50" style="right:240px;">{{ $rek1 }}</td>
                        <td class="col-fixed-right px-3 py-3 text-left bg-purple-50" style="right:120px;">{{ $rek2 }}</td>
                        <td class="col-fixed-right px-3 py-3 text-left bg-purple-50" style="right:0;">{{ $rek3 }}</td>
                    </tr>
                @empty
                    <tr>
...
<style>
    ...
    .hasil-table-wrapper::-webkit-scrollbar { height: 6px; }
    .hasil-table-wrapper::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 99px; }
    .hasil-table-wrapper::-webkit-scrollbar-thumb { background: #c7c7c7; border-radius: 99px; }
    .hasil-table-wrapper::-webkit-scrollbar-thumb:hover { background: #a0a0a0; }
</style>
